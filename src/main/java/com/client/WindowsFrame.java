...
import com.client.view.util.StatusProgressPanel;
	...
	public WindowsFrame() {
		...
		projectJComboBox.addPopupMenuListener(new PopupMenuListener(){  
            public void popupMenuWillBecomeVisible(PopupMenuEvent e) { 
            	...
            	if(!DataCache.PROJECT_MAP.isEmpty()){
            		for(Entry<String, Object> project : DataCache.PROJECT_MAP.entrySet()){
            			String item = project.getValue().toString() + ":" + project.getKey();
            			projectJComboBox.addItem(item);
            			if(null != DataCache.PROJECT_NAME && DataCache.PROJECT_NAME.length() > 0 && DataCache.PROJECT_NAME.equals(item.toString())){
            				projectJComboBox.setSelectedItem(DataCache.PROJECT_NAME);
            			}
            		}
            	} else if(null != DataCache.PROJECT_NAME){
            		projectJComboBox.addItem(DataCache.PROJECT_NAME);
            		projectJComboBox.setSelectedItem(DataCache.PROJECT_NAME);
            	}
            }  
		...
		netJComboBox = ViewModules.createComboBox(DataCache.DEVICES_MAP.keySet().toArray());
